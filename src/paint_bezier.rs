...
impl Default for PaintBezier {
    fn default() -> Self {
        Self {
            ...
            bounding_box_stroke: Stroke::new(
                0.0,
                Color32::LIGHT_GREEN.linear_multiply(0.25),
            ),
        }
    }
}
...
impl PaintBezier {
    pub fn ui_control(&mut self, ui: &mut egui::Ui) {
        ui.collapsing("Colors", |ui| {
            ...
            egui::stroke_ui(
                ui,
                &mut self.bounding_box_stroke,
                "Bounding Box Stroke",
            );
        });

        ui.collapsing("Global tessellation options", |ui| {
            let mut tessellation_options =
                ui.ctx().tessellation_options(|to| *to);
    ...
    pub fn ui_content(&mut self, ui: &mut Ui) -> egui::Response {
        ...
        // Transform coordinate system from one to other
        let to_screen = emath::RectTransform::from_to(
            Rect::from_min_size(Pos2::ZERO, response.rect.size()),
            response.rect,
        );

        // Radius of control-circles
        let control_point_radius = 8.0;

        // Get vector with circle-control shapes
        let control_point_shapes: Vec<Shape> = self
            ...
            .map(|(i, point)| {
                ...
                // Transform point to local space, from POS::ZERO
                let point_in_screen = to_screen.transform_pos(*point);
                let point_rect = Rect::from_center_size(point_in_screen, size);
                // Generate unique id for point
                let point_id = response.id.with(i);
                // Create drag response for point
                let point_response =
                    ui.interact(point_rect, point_id, Sense::drag());

                // Get new position and assign it by delta
                *point += point_response.drag_delta();
                // Clamp in our paint-space
                *point = to_screen.from().clamp(*point);

                // Again transform already moved point
                let point_in_screen = to_screen.transform_pos(*point);
        ...
        // Transform our control points to local coordinate space
...
impl PaintBezier {
    fn ui(&mut self, ui: &mut Ui) {
        ui.vertical_centered(|ui| ui.label("I'm label!"));
        self.ui_control(ui);
