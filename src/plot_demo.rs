...
use egui::plot::{GridInput, GridMark, LinkedAxisGroup, LinkedCursorsGroup};
use egui::*;
use plot::{
    Arrows, Bar, BarChart, BoxElem, BoxPlot, BoxSpread, CoordinatesFormatter,
    Corner, HLine, Legend, Line, LineStyle, MarkerShape, Plot, PlotImage,
        ...
        ui.horizontal(|ui| {
            ...
            ui.collapsing("Instructions", |ui| {
                ...
                if cfg!(target_arch = "wasm32") {
                    ui.label("Zoom with ctrl / CMD + pointer wheel, or with pinch gesture.");
                } else if cfg!(target_os = "macos") {
                    ui.label("Zoom with ctrl / CMD + scroll.");
                } else {
...
impl CustomAxisDemo {
    ...
    fn logistic_fn() -> Line {
        ...
        let values = PlotPoints::from_explicit_callback(
            move |x| {
                1.0 / (1.0
                    + (-2.5 * (x / CustomAxisDemo::MINS_PER_DAY - 2.0)).exp())
            },
            days(0.0)..days(5.0),
            100,
        );
    ...
    fn ui(&mut self, ui: &mut Ui) -> Response {
        ...
        ui.horizontal(|ui| {
            Plot::new("linked_axis_1")
                ...
                .width(250.0)
                .height(250.0)
                .link_axis(LinkedAxisGroup::new(self.link_x, self.link_y))
                .link_cursor(LinkedCursorsGroup::new(
                    self.link_cursor_x,
                    self.link_cursor_y,
                ))
...
impl ItemsDemo {
    fn ui(&mut self, ui: &mut Ui) -> Response {
        ...
        plot.show(ui, |plot_ui| {
            ...
            plot_ui.text(
                Text::new(PlotPoint::new(-3.0, -3.0), "wow").name("Text"),
            );
            plot_ui.text(
                Text::new(PlotPoint::new(-2.0, 2.5), "so graph").name("Text"),
            );
            plot_ui.text(
                Text::new(PlotPoint::new(3.0, 3.0), "much color").name("Text"),
            );
            plot_ui.text(
                Text::new(PlotPoint::new(2.5, -2.0), "such plot").name("Text"),
            );
...
impl ChartsDemo {
    ...
    fn bar_gauss(&self, ui: &mut Ui) -> Response {
        ...
        Plot::new("Normal Distribution Demo")
            .legend(Legend::default())
            .clamp_grid(true)
            .allow_zoom(self.allow_zoom.x)
            .allow_drag(self.allow_drag.x)
    ...
    fn box_plot(&self, ui: &mut Ui) -> Response {
        ...
        let mut box2 = BoxPlot::new(vec![
            BoxElem::new(1.0, BoxSpread::new(0.2, 0.5, 1.0, 2.0, 2.7))
                .name("Day 1"),
            ...
            BoxElem::new(5.0, BoxSpread::new(1.3, 2.0, 2.3, 2.9, 4.0))
                .name("Day 3"),
        ])
        .name("Experiment B");

        let mut box3 = BoxPlot::new(vec![
            BoxElem::new(1.5, BoxSpread::new(2.1, 2.2, 2.6, 2.8, 3.0))
                .name("Day 1"),
            BoxElem::new(3.5, BoxSpread::new(1.3, 1.5, 1.9, 2.2, 2.4))
                .name("Day 2"),
            BoxElem::new(5.5, BoxSpread::new(0.2, 0.4, 1.0, 1.3, 1.5))
                .name("Day 3"),
        ])
        .name("Experiment C");
